...
export function useTaskManagementApp() {
  ...
  const [timers, setTimers] = useState<Record<string, TimerState>>(INITIAL_TIMERS);
  ...
  useEffect(() => {
    setTimers(previousTimers => {
      ...
      tasks.forEach(task => {
        ...
        nextTimers[task.id] = createTimerState(getTaskDurationMinutes(task) || 60, {
  ...
  const normalizeBackendUser = (backendUser: BackendUser, sourceUsers: User[]) => {
  ...
  const mergeUsersWithBackend = (sourceUsers: User[], backendUsers: BackendUser[]) => {
    const normalizedUsers = backendUsers.reduce<User[]>((collection, backendUser) => {
      const normalizedUser = normalizeBackendUser(backendUser, sourceUsers);
      const identityKey = getUserIdentityKey(normalizedUser);

      if (!collection.some(user => getUserIdentityKey(user) === identityKey)) {
        collection.push(normalizedUser);
      }

      return collection;
    }, []);
    const syncedEmails = new Set(normalizedUsers.map(user => user.email.toLowerCase()));
  ...
  const getBackendUserId = useCallback(
    (userId: string) => users.find(user => user.id === userId)?.backendId ?? userId,
    [users],
  );
  ...
  const normalizeBackendTask = (
    ...
    return {
      ...
      totalBreakSeconds: backendTask.totalBreakSeconds ?? existingTask?.totalBreakSeconds ?? 0,
      pauseStartedAt: backendTask.pauseStartedAt || existingTask?.pauseStartedAt,
      extensionHistory: backendTask.extensionHistory ?? existingTask?.extensionHistory ?? [],
  ...
  const fetchAllUsers = async (
    token: string,
    options?: { silent?: boolean; focusUser?: BackendUser | null; replace?: boolean },
  ) => {
    if (!options?.silent) {
    ...
    try {
      ...
      do {
        const response = await apiRequest(`/auth/get_all_users?limit=${limit}&page=${page}`, {
          method: 'GET',
          headers: {
  ...
  const manageTasks = useMemo(() => {
    switch (manageTab) {
      ...
      case 'review':
        return assignedByMe.filter(task => task.status === 'under_review' || task.status === 'rejected');
      default:
        return [];
    }
  ...
  const getUserName = useCallback(
    (userId: string) =>
      users.find(user => user.id === userId || user.backendId === userId)?.name ?? userId,
    [users],
  );
  ...
  const isNotificationForCurrentUser = useCallback(
    (notification: AppNotification) => {
      ...
      return recipientKeys.some(recipientKey => currentUserKeys.includes(recipientKey));
    },
    [currentUser, tasks],
  );
  ...
  const addTaskNotification = (
    ...
    const nextNotification = createInAppNotification(type, title, message, taskId, options);
  ...
  const visibleNotifications = useMemo(() => {
    ...
    const currentUserKeys = new Set(
      [currentUser.id, currentUser.backendId].filter(
        (value): value is string => Boolean(value?.trim()),
      ),
    );
    ...
    tasks.forEach(task => {
      ...
      if (isAssignee) {
        taskNotifications.push({
          ...
          message: `${getUserName(task.assignedBy)} assigned '${task.title}' to you.`,
          taskId: task.id,
        ...
        if (task.feedback?.trim()) {
          taskNotifications.push({
            ...
            type: 'task_returned',
            title: 'Task Returned',
            message: `${getUserName(task.assignedBy)} returned '${task.title}' for changes.`,
            taskId: task.id,
            targetScreen: 'activeTask',
            timestamp: formatNotificationTimestamp(task.createdAt),
            read: readIdSet.has(`task-returned-${task.id}-${task.feedback.trim()}`),
          });
        }
      }
      ...
      if (isAssigner && task.status === 'under_review') {
        taskNotifications.push({
          ...
          message: `${getUserName(task.assignedTo)} sent '${task.title}' for your review.`,
          taskId: task.id,
          targetScreen: 'reviewTask',
          timestamp: formatNotificationTimestamp(task.createdAt),
          read: readIdSet.has(`task-review-${task.id}-${task.reviewComment ?? ''}`),
        });
      }
      ...
      if (isAssigner && task.status === 'rejected') {
        taskNotifications.push({
          ...
          targetScreen: 'reviewTask',
          timestamp: formatNotificationTimestamp(task.createdAt),
          read: readIdSet.has(`task-rejected-${task.id}-${task.reviewComment ?? ''}`),
        });
      }
      ...
      if (isAssigner && task.status === 'completed') {
        taskNotifications.push({
          ...
          timestamp: formatNotificationTimestamp(task.completedAt ?? task.createdAt),
          read: readIdSet.has(`task-completed-${task.id}-${task.completedAt ?? ''}`),
        });
      }
    });
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      try {
        await fetchRelatedTasks(token, { silent: true, sourceUsers: syncedUsers });
      } catch (error) {
  ...
  const handleVerifyForgotPasswordOtp = async () => {
    ...
    try {
      ...
      setForgotPasswordMessage(response?.message ?? 'OTP verified successfully.');
...
const showTimedSuccess = (
  ...
  const handleDeleteUser = async (userId: string) => {
    ...
    try {
      ...
      if (selectedUserId === userId || userToDelete?.backendId === selectedUserId) {
        setSelectedUserId(null);
      }
      ...
      setTasks(prev =>
        prev.map(task => ({
          ...task,
          assignedTo: task.assignedTo === userId ? replacementUserId : task.assignedTo,
          assignedBy: task.assignedBy === userId ? replacementUserId : task.assignedBy,
        })),
      );
  ...
  const handlePersistTask = async (mode: 'create' | 'edit') => {
    ...
    if (
      ...
      setTaskActionError('Fill in the task title, due date, task time, and assignee.');
      return;
    }
    ...
    try {
      ...
      if (mode === 'create') {
        ...
        createdTaskId =
          response?.data?._id ?? response?.result?._id ?? response?.results?._id ?? '';
      } else {
      ...
      if (mode === 'edit' && selectedTask) {
        setTimers(prev => ({
          ...prev,
          [selectedTask.id]: createTimerState(getTaskDurationMinutes(payload) || 60),
        }));
      }
      ...
      if (mode === 'create' || mode === 'edit') {
        addTaskNotification(
          'task_assigned',
          'New Task Assigned',
          `${currentUser?.name ?? 'Someone'} assigned '${payload.title}' to you.`,
  ...
  const persistTaskMutation = async (
    ...
    options?: { successMessage?: string; nextScreen?: Screen; refreshTasks?: boolean },
  ) => {
    if (!authToken) {
    ...
    try {
      await apiRequest(`/tasks/${taskId}`, {
        ...
        body: JSON.stringify({
          ...
          assignedTo: getBackendUserId(changes.assignedTo ?? existingTask.assignedTo),
          status: changes.status ?? existingTask.status,
          accepted: changes.accepted ?? existingTask.accepted ?? false,
          reviewComment: changes.reviewComment ?? existingTask.reviewComment ?? '',
          ...
          pauseStartedAt: changes.pauseStartedAt ?? existingTask.pauseStartedAt ?? '',
          extensionHistory: changes.extensionHistory ?? existingTask.extensionHistory ?? [],
          completionStatusNote:
            changes.completionStatusNote ?? existingTask.completionStatusNote ?? '',
        }),
      });
      ...
      if (options?.successMessage) {
        showTimedSuccess(options.successMessage, options.nextScreen ?? 'dashboard');
      }
    ...
    setTimers(prev => ({
      ...
      ...Object.fromEntries(
        previousActiveTaskIds.map(id => {
          ...
          const nextPauseHistory = [...timer.pauseHistory, getTimestampLabel(new Date(pausedAt))];
    ...
    await Promise.all([
      ...
      ...previousActiveTaskIds.map(id =>
        persistTaskMutation(id, {
          ...
          accepted: true,
          isPaused: true,
          pauseHistory: [...(timers[id]?.pauseHistory ?? []), getTimestampLabel()],
      ...
      prev.map(existingTask =>
        existingTask.id === taskId
          ? {
              ...
              isPaused: true,
              pauseHistory: currentTimer.pauseHistory ?? existingTask.pauseHistory ?? [],
              totalBreakSeconds,
              pauseStartedAt: undefined,
              extensionHistory:
                currentTimer.extensionHistory ?? existingTask.extensionHistory ?? [],
              completionStatusNote,
            }
          : existingTask,
      ),
  ...
  const handleRejectTask = async (taskId = selectedTaskId ?? '', reason = '') => {
    if (!taskId) {
      return;
    }
  ...
  const handleReturnForChanges = async (taskId: string, comment: string) => {
    ...
    if (task) {
      addTaskNotification(
        'task_returned',
        'Task Returned',
        `${currentUser?.name ?? 'Someone'} returned '${task.title}' for changes.`,
        taskId,
        {
          recipientUserId: task.assignedTo,
          recipientBackendUserId: getBackendUserId(task.assignedTo),
  ...
  const handleAddMoreTime = async (taskId: string, minutes: number, reason = '') => {
    ...
    const extensionLabel = trimmedReason
      ? `Extra time taken: ${formatDurationLabel(minutes)} • Reason: ${trimmedReason}`
    ...
    const nextExtensionHistory = [...currentTimer.extensionHistory, extensionLabel];
  ...
  const handleForegroundPushMessage = useEffectEvent((notification: AppNotification) => {
  ...
  const handleOpenedPushMessage = useEffectEvent((notification: AppNotification) => {
    if (!isNotificationForCurrentUser(notification)) {
      return;
    }
  ...
  const currentTaskList = () => {
    switch (taskTab) {
      ...
      case 'review':
        return myTasks.filter(task => task.status === 'under_review' || task.status === 'rejected');
      case 'complete':
  ...
  const homeStats = [
    { label: 'Pending', value: myTasks.filter(task => task.status === 'pending').length },
    {
      label: 'Review',
    ...
    { label: 'Done', value: myTasks.filter(task => task.status === 'completed').length },
  ...
  const renderDashboardNavItem = (label: string, tab: DashboardTab, icon: string) => (
    <DashboardNavItem
      key={tab}
      label={label}
      tab={tab}
