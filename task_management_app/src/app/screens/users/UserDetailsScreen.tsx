...
export function UserDetailsScreen(props: UserDetailsScreenProps) {
  const { selectedUser, tasks, goBack, renderDashboardNavItem, getUserName } = props;
  ...
  const userTasks = useMemo(
    () =>
      tasks.filter(
        task =>
          task.assignedTo === selectedUser?.id || task.assignedTo === selectedUser?.backendId,
      ),
    [selectedUser?.backendId, selectedUser?.id, tasks],
  );
