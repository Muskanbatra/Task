...
export function ManageUsersScreen(props: ManageUsersScreenProps) {
  ...
  const uniqueUsers = useMemo(() => {
    ...
    return users.filter(user => {
      const identityKey = user.backendId || user.email.toLowerCase() || user.id;
    ...
    () =>
      uniqueUsers.map(user => {
        const assignedTasks = tasks.filter(
          task =>
            task.assignedTo === user.id || task.assignedTo === user.backendId,
        );
        ...
        return {
          ...
          pending: assignedTasks.filter(task => task.status === 'pending')
            .length,
          active: assignedTasks.filter(task => task.status === 'in_progress')
            .length,
          review: assignedTasks.filter(
            task => task.status === 'under_review' || task.status === 'rejected',
          ).length,
          done: assignedTasks.filter(task => task.status === 'completed')
            .length,
          tasks: assignedTasks,
        };
      }),
